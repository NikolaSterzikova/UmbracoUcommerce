        ...
        public override ActionResult Index(RenderModel model)
        {
            ...
            foreach (var orderLine in basket.OrderLines)
            {
                var orderLineViewModel = new OrderlineViewModel
                {
                    ...
                    Total = "100",//new Money(orderLine.Total.GetValueOrDefault(), basket.BillingCurrency).ToString(),
                    Discount = orderLine.Discount,
                    Tax = "10",//new Money(orderLine.VAT, basket.BillingCurrency).ToString(),
                    Price = "100",//new Money(orderLine.Price, basket.BillingCurrency).ToString(),
                    ProductUrl = CatalogLibrary.GetNiceUrlForProduct(CatalogLibrary.GetProduct(orderLine.Sku)),
                    PriceWithDiscount = "100",//new Money(orderLine.Price - orderLine.UnitDiscount.GetValueOrDefault(), basket.BillingCurrency).ToString(),
