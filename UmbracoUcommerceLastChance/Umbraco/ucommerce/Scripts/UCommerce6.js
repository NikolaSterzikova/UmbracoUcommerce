var UCommerceClientMgr = {
  ...
  openModal: function (url, name, width, height) {
    ...
    top.angular.element('.umb-modal').css('margin-left', marginPx + 'px').css('z-index', 2000)
  ...
  getCurrentNodeId: function () {
    return getUmbClientMgr()
      .mainTree()
      .getActionNode().nodeId
  },

  contentFrame: function (url) {
    top.angular
      .element('#umbracoMainPageBody')
      .scope()
      .$apply(function () {
        top.UmbClientMgr.contentFrame(url)
        top.angular
          .element(top.angular.element('#umbracoMainPageBody'))
          .injector()
          .get('$route')
          .reload()
      })
  ...
  updateCurrentNodeText: function (nodeText) {
    getUmbClientMgr()
      .mainWindow()
      .jQuery('.sprTree.noSpr.clicked > div')
      .text(nodeText)
  },
  ...
  showSpeechBubble: function (icon, header, body) {
    getUmbClientMgr()
      .mainWindow()
      .UmbSpeechBubble.ShowMessage(icon, header, body)
  },

  setFocusToTreeNode: function (treeNode) {
    getUmbClientMgr()
      .mainTree()
      .selectNode(treeNode, false, true)
  },
  ...
  showPopupWindow: function (url, width, height, saveFunction) {
    getUmbClientMgr().openModalWindow(
      this._itemPickerUrl,
      this._label,
      this._showHeader,
      this._width,
...
function sortCategoryThis () {
  UCommerceClientMgr.openModal(
    'ucommerce/catalog/dialogs/SortCatalog.aspx?id=' +
      UCommerceClientMgr.getCurrentNodeId(),
    'Sort',
    600,
    450
  )
...
function refreshChildren (n, args) {
  parent.setTimeout(function () {
    ...
    var treeChildren = treeElement.children()[0].children[0].children[1]
      .children
